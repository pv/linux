				       ...
				       u8 cis, int *pos)
{
	...
	c = hci_cig_list_find(cig_list, cig);
	if (!c)
		return NULL;

	for (i = 0; i < c->num_cis; ++i)
		if (c->qos[i].cis == cis)
			break;
	if (i == c->num_cis)
		return NULL;
...
static int hci_set_cis_params(struct hci_dev *hdev, struct bt_iso_qos *qos)
{
	...
	entry->qos[i] = *qos;
#else
	/* TODO: not clear if the above reading of the spec is too tight. If the
	 * command completely replaces the previous CIG configuration and
	 * invalidates the old CIS handles, we should instead get the list from
	 * currently active hci_conn like so:
	 */
	{
		struct hci_conn *conn;
		...
		list_for_each_entry_rcu(conn, &hdev->conn_hash.list, list) {
			if (conn->type != ISO_LINK || conn->state != BT_BOUND ||
			    conn->iso_qos.cig != cig ||
			    conn->iso_qos.cis == cis)
				continue;
			...
			entry->qos[i] = conn->iso_qos;
			entry->handles[i] = conn->handle;
			++i;
		}
...
bool hci_iso_setup_path(struct hci_conn *conn)
{
	...
	if (conn->role == HCI_ROLE_MASTER) {
		...
		cig = hci_cig_list_find_cis(&hdev->central_cig_list,
					    ...
					    &i);
		if (cig)
			existing = cig->data_path[i];
	}

	memset(&cmd, 0, sizeof(cmd));

	if (conn->iso_qos.out.sdu && !(existing & 0x1)) {
	...
	if (conn->iso_qos.in.sdu && !(existing & 0x2)) {
		cmd.handle = cpu_to_le16(conn->handle);
